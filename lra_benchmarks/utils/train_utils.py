...
from lra_benchmarks.models.wavspa import wavspa_learn
...
def get_model(model_type, create_model_fn, model_kwargs, *create_model_args):
    ...
    if model_type == 'transformer':
        ...
        return create_model_fn(transformer.TransformerDualEncoder, model_kwargs,
                               *create_model_args)
        ...
        return create_model_fn(longformer.LongformerEncoder, model_kwargs,
                               *create_model_args)
    elif model_type == 'wavspa_learn':
        return create_model_fn(wavspa_learn.HippoEncoder, model_kwargs,
                               *create_model_args) 
    elif model_type == 'wavspa_learn_dual':
        return create_model_fn(wavspa_learn.HippoDualEncoder, model_kwargs,
                               *create_model_args)
    else:
...
def compute_weighted_cross_entropy(logits, targets, num_classes, weights=None):
    ...
    loss = onehot_targets * nn.log_softmax(logits)
...
def compute_f1_score(logits, targets, label=1):
    """Compute f1 score for log probs and targets.
    ...
    tp = jnp.sum((actual==label) & (predicted==label))
    fp = jnp.sum((actual!=label) & (predicted==label))
    fn = jnp.sum((predicted!=label) & (actual==label))
    tn = jnp.sum((actual!=label) & (predicted!=label))
    
    #tn, fp, fn, tp = confusion_matrix(y_true=targets, y_pred=jnp.argmax(logits, axis=-1)).ravel()
    normalizing_factor = onp.prod(logits.shape[:-1])

    return tn, fp, fn, tp, normalizing_factor
