...
class LongformerAttention(nn.Module):
    ...
    def __call__(self, 
        ...
        query_sw = nn.DenseGeneral(axis=-1,
                                   features=(self.num_heads, head_dim),
                                   kernel_init=self.kernel_init,
                                   bias_init=self.bias_init,
                                   use_bias=self.bias,
                                   precision=self.precision, dtype=self.dtype, name='query_sliding_window')(inputs_q)
        key_sw = nn.DenseGeneral(axis=-1,
                                   features=(self.num_heads, head_dim),
                                   kernel_init=self.kernel_init,
                                   bias_init=self.bias_init,
                                   use_bias=self.bias,
                                   precision=self.precision, dtype=self.dtype, name='key_sliding_window')(inputs_kv)
        value_sw = nn.DenseGeneral(axis=-1,
                                   features=(self.num_heads, head_dim),
                                   kernel_init=self.kernel_init,
                                   bias_init=self.bias_init,
                                   use_bias=self.bias,
                                   precision=self.precision, dtype=self.dtype, name='value_sliding_window')(inputs_kv)

        query_global = nn.DenseGeneral(axis=-1,
                                   features=(self.num_heads, head_dim),
                                   kernel_init=self.kernel_init,
                                   bias_init=self.bias_init,
                                   use_bias=self.bias,
                                   precision=self.precision, dtype=self.dtype, name='query_global')(inputs_q)
        key_global = nn.DenseGeneral(axis=-1,
                                   features=(self.num_heads, head_dim),
                                   kernel_init=self.kernel_init,
                                   bias_init=self.bias_init,
                                   use_bias=self.bias,
                                   precision=self.precision, dtype=self.dtype, name='key_global')(inputs_kv)
        value_global = nn.DenseGeneral(axis=-1,
                                   features=(self.num_heads, head_dim),
                                   kernel_init=self.kernel_init,
                                   bias_init=self.bias_init,
                                   use_bias=self.bias,
                                   precision=self.precision, dtype=self.dtype, name='value_global')(inputs_kv)
        ...
        x = jnp.where(global_mask[:, :, jnp.newaxis,
                      jnp.newaxis], x_global, x_sw)
