...
class PerformerAttn(nn.Module):
    ...
    lax_scan_unroll: int = 1
    qkv_normalizarion: bool = False
    ...
    def __call__(self, x, mask=None, deterministic=False):
        ...
        if self.generalized:
            attention_fn = make_fast_generalized_attention(self.qkv_dim // self.num_heads, 
                                                           nb_features=self.nb_features,
                                                           unidirectional=self.unidirectional, 
                                                           lax_scan_unroll=self.lax_scan_unroll,
                                                           qkv_normalizarion=self.qkv_normalizarion)
        else:
            attention_fn = make_fast_softmax_attention(self.qkv_dim // self.num_heads, 
                                                       nb_features=self.nb_features,
                                                       unidirectional=self.unidirectional, 
                                                       lax_scan_unroll=self.lax_scan_unroll)
...
def nonnegative_softmax_kernel_feature_creator(data,
    ...
    data_thick_random_matrix = jnp.zeros(data_mod_shape, dtype=jnp.float32) + projection_matrix.astype(jnp.float32)
...
def make_fast_generalized_attention(qkv_dim,
    ...
    attention_fn = FastAttentionviaLowRankDecomposition(
        ...
        qkv_normalizarion=qkv_normalizarion).dot_product_attention
    return attention_fn
...
class FastAttentionviaLowRankDecomposition(FastAttention):
    ...
    def __init__(self,
                 ...
                 unidirectional,
                 lax_scan_unroll=1,
                 qkv_normalizarion=False):  # For optimal GPU performance, set to 16.
        ...
        self.qkv_normalizarion = qkv_normalizarion
    ...
    def dot_product_attention(self,
        ...
        ## qkv_normalizarion: ADDING NORMALIZATION OVER HERE
        if self.qkv_normalizarion:
            query = nn.LayerNorm(dtype=jnp.float32)(query)
            key = nn.LayerNorm(dtype=jnp.float32)(key)
            value = nn.LayerNorm(dtype=jnp.float32)(value)
            '''
            query = query / jnp.linalg.norm(query, ord=2, axis=-1, keepdims=True)
            key = key / jnp.linalg.norm(key, ord=2, axis=-1, keepdims=True)
            value = value / jnp.linalg.norm(value, ord=2, axis=-1, keepdims=True)
            '''
        ...
        #key_prime = key_prime.astype(self.dtype)
        if self.unidirectional:
            ...
            if not self.renormalize_attention:
                ...
                thick_all_ones = jnp.zeros(key.shape[0:-1], dtype=dtype) + jnp.ones(
                    key_extra.shape[0:len(axis)], dtype=dtype)
                contract_key = tuple(
                    range(len(batch_dims),
                          len(batch_dims) + len(axis)))
