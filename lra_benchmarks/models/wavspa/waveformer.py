...
from lra_benchmarks.models.wavspa.configuration_waveformer import WaveformerConfig as BertConfig
...
_CHECKPOINT_FOR_DOC = "waveformer"
_CONFIG_FOR_DOC = "WaveformerConfig"
...
class FlaxBertSelfAttention(nn.Module):
    ## Warning:
    ## This is Waveformer Attention Module
    config: BertConfig
    causal: bool = False
    ...
    def setup(self):
        ## Settings for WavSpa
        self.adawave = self.param('adawave', db_init, (self.config.wlen, self.config.wav_dim), self.dtype)
        
        ## Regular BERT Setting 
    ...
    def __call__(
        ...
        if is_cross_attention or self.causal:
        ...
        if attention_mask is None:
            ...
            src_padding_mask = (attention_mask > 0)[..., None]
            x = jnp.where(src_padding_mask, hidden_states, 0)
        
        ## Waveformer Attention
        wavelet = self.adawave
        
        z = jwt.wavedec_learn(x, wavelet, level=self.config.level)
        #z = jwt.wavedec2(x, pywt.Wavelet(wavelet), level=self.config.level, mode="reflect")
        for level in range(len(z)):
            z[level] = z[0].astype(jnp.float32)
            z[level] = nn.SelfAttention(num_heads=self.config.num_attention_heads,
                                    dtype=self.dtype,
        ...
        z = jwt.waverec_learn(z, wavelet)[:,:x.shape[1],:]
...
class FlaxWBertForMaskedLM(FlaxBertPreTrainedModel):
    module_class = FlaxBertForMaskedLMModule


append_call_sample_docstring(
    FlaxWBertForMaskedLM, _TOKENIZER_FOR_DOC, _CHECKPOINT_FOR_DOC, FlaxMaskedLMOutput, _CONFIG_FOR_DOC
)
