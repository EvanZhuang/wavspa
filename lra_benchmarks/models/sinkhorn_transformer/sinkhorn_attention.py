...
def sinkhorn_operator(log_alpha, n_iters=1, temp=1.0, clip=1.0, causal=True):
    ...
    for _ in range(n_iters):
        if causal:
            ...
            log_alpha -= jnp.reshape(reduce_logsumexp(log_alpha,
                                     axis=2), (-1, n, 1))
            log_alpha -= jnp.reshape(reduce_logsumexp(log_alpha,
                                     axis=1), (-1, 1, n))
...
def local_dot_product_attention(query,
    ...
    wv_contracting_dims = (norm_dims, range(
        value.ndim - len(axis), value.ndim))
    y = lax.dot_general(
        attn_weights,
...
class SinkhornAttention(nn.Module):
    ...
    def apply(self,
        ...
        if cache:
            assert isinstance(
                cache, Cache), 'cache must be an instance of Cache'
            if self.is_initializing():
                ...
                value = lax.dynamic_update_slice(
                    cache_entry.value, value, indices)
        ...
        sorted_value = jnp.einsum(
            'bskhd,bnhl->bsnhd', block_value, permutation)
        ...
        if padding_mask is not None:
            ...
            padding_mask = make_padding_mask(
                ...
                query_shape=(bs * num_query_blocks,
                             block_size, num_heads, head_dim),
                key_shape=(bs * num_kv_blocks, block_size,
                           num_heads, head_dim),
        ...
        if mask_components:
            ...
            attention_bias = lax.select(
                attention_mask > 0, jnp.full(
                    attention_mask.shape, 0.).astype(dtype),
